...
def build_feature_matrix(df_1h: pd.DataFrame, df_4h: pd.DataFrame,
    ...
    # ICT recientes (ventana móvil 3-5 velas)
    ...
    # OB cercano (precio dentro de ±1% del OB)
    ...
    # OB 4h cerca del precio
    if 'ob_bullish_4h' in df.columns:
        df['ob_bull_4h_int'] = df['ob_bullish_4h'].fillna(False).astype(int)
        df['ob_bear_4h_int'] = df['ob_bearish_4h'].fillna(False).astype(int) if 'ob_bearish_4h' in df.columns else 0
...
def train(target_hours: int = 4) -> dict:
    ...
    # TimeSeriesSplit — nunca shuffle con series temporales
    ...
    log.info("Validación TimeSeriesSplit (5 folds)...")
